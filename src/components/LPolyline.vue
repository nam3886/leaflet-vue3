...
import { DomEvent, polyline } from 'leaflet'
...
type Props = {
  readonly latLngs: Parameters<typeof polyline>[0]
  readonly hidden?: boolean
  readonly layerName?: string
  readonly layerType?: 'base' | 'overlay'
  readonly options?: Parameters<typeof polyline>[1]
}
const props = defineProps<Props>()
const emit = defineEmits<{
  (e: 'ready', mapObject: ReturnType<typeof polyline>): void
  (e: 'update:visible', isVisible: boolean): void
}>()
defineOptions({ inheritAttrs: false, name: 'LPolyline' })

const instance = getCurrentInstance()
const attrs = useAttrs()

const mapObject = ref<ReturnType<typeof polyline>>()
...
function create() {
  ...
  mapObject.value = polyline(latLngs.value, options.value)
